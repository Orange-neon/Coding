...
int main() {
    ...
    vector<vector<int>> badmilks(n);
    map<int, set<int>> milkdranks;
    for (int i = 0; i < d; ++i) {
        int p, m, t;
        cin >> p >> m >> t;
        records.at(p - 1).push_back({t, m});
    ...
    for (const auto& pair : sicked) {
        for (auto milk : records.at(pair.first)) {
            if (milk.first < pair.second) {
                badmilks.at(pair.first).push_back(milk.second);
            }
        }
    }
    ...
    for (auto possibiles : badmilks) {
        for (int leche : possibiles) {
            ++badMilkTrack[leche];
            if (badMilkTrack.at(leche) == s) {
                ans = max(ans, int(milkdranks.at(leche).size()));
            }
        }
    }
    cout << ans;
